...
import {
  ...
  Typography, useTheme, useMediaQuery, Paper, alpha
} from '@mui/material';
import { useSelector, useDispatch } from 'react-redux';
import { useNavigate } from 'react-router-dom';
import { selectActiveStep, resetCheckout } from '@/redux/slices/checkoutSlice';
...
const CheckoutPage = () => {
  ...
  useEffect(() => {
    if (cartItems.length === 0 && activeStep === 0) {
      // navigate('/'); // Commented out for dev so I can see the page
    }
              ...
              <Stepper activeStep={activeStep} alternativeLabel={!isMobile} orientation={isMobile ? 'vertical' : 'horizontal'}>
                {steps.map((label) => (
                  <Step key={label}>
                    ...
                    >
                      <Typography variant="body2" fontWeight={activeStep >= steps.indexOf(label) ? 700 : 400}>
                        {label}
                      </Typography>
                    </StepLabel>
                  </Step>
