...
import { 
  ...
  Stack, Divider, Grid, alpha, useTheme 
} from '@mui/material';
import { 
...
const OrderConfirmationPage = () => {
  ...
  // In a real app, you'd fetch the order if state is missing
        ...
        >
          ...
          <Box sx={{ mb: 4, p: 3, bgcolor: alpha(theme.palette.primary.main, 0.03), borderRadius: 4, border: '1px solid', borderColor: alpha(theme.palette.primary.main, 0.1) }}>
            ...
            <Typography variant="h5" fontWeight={900} color="primary">
              #{order.id?.toString().slice(-8).toUpperCase() || 'ORD12345678'}
            </Typography>
          </Box>
          ...
          <Stack spacing={3} sx={{ textAlign: 'left', mb: 4 }}>
            ...
            <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
              ...
              <Typography variant="body2" fontWeight={800} align="right">{order.address?.street || order.address}</Typography>
            </Box>
            <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
